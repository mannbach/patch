...
The high-level execution scripts in `/scripts/` read configuration parameters from the command line.
If no argument is given, a default value is typically taken from `patch/constants.py`.
These scripts also provide a description when adding the `-h` flag, e.g.,
```bash
...
The structure of each JSON file is explained by the example of `data/graphs/N-200_m-2_f-0.3_h-0.2_tau-0.0_lfm-l-homophily_lfm-g-homophily_r-0.json`:
...
They can be imported in an arbitrary Python script as (see [notebooks](#notebooks) for examples):
```python
from patch_workshop.utils import create_graph
```

### scripts
Contains Python scripts to simulate many networks, as well as code to compute the aggregate statistics required to produces the presented plots.
